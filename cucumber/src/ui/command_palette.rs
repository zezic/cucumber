...
impl CommandPalette {
    ...
    fn alternatives_ui(
        ...
        for (i, fuzzy_match) in commands_that_match(&query).iter().enumerate() {
            ...
            let horizontal_padding = Vec2::new(padding, 0.0);
            let text_rect = Align2::LEFT_CENTER.anchor_rect(egui::Rect::from_min_size(
                rect.left_center() + horizontal_padding,
                galley.size(),
            ));
            ui.painter()
            ...
            ui.painter().text(
                rect.right_center() - horizontal_padding,
                Align2::RIGHT_CENTER,
                kb_shortcut_text,
                font_id.clone(),
                if selected {
